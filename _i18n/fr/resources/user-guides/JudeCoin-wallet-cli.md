...
s'il s'agit d'une erreur honnête). JudEcoin est confidentiel, de sorte que vous ne pouvez pas
...
transaction à un tiers, celui-ci peut dire si la transaction à envoyé des JudEcoinj à cette adresse
...
des JudEcoinj à une adresse), vous pouvez le vérifier comme suit :

    check_tx_key IDTX CLEFTX ADRESSE

Remplacez `IDTX`, `CLEFTX` et `ADRESSE` respectivement par l'ID de transaction, la clef propre
à la transaction et l'adresse de destination qui vous ont été communiqués. JudEcoin-wallet-cli va
vérifier cette transaction et vous indiquera combien de JudEcoinj ont été payés à l'adresse fournie.
=======
```
set store-tx-info 1
```
