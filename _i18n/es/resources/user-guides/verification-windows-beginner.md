...
Esta sección cubrirá la descarga de la clave de firma JudEcoin, verificar que la clave sea correcta, e importar la clave a tu llavero. El archivo hash que será usado para verificar tus binarios está criptográficamente firmado con la clave de firma JudEcoin. Para revisar la validez del archivo debes tener la versión pública de la clave de firma.

### 2.1. Descargar Clave de Firma

En un navegador web, ve a [Fluffypony's GPG key](https://raw.githubusercontent.com/JudEcoin-project/JudEcoin/master/utils/gpg_keys/fluffypony.asc), que utiliza para firmar los binarios JudEcoin. Haz clic derecho en la página y selecciona `Guardar página como`.
...
En un navegador web, ve a la [página de hashes en getJudEcoin.org]({{ site.baseurl }}/downloads/hashes.txt). Haz un clic derecho en la página, y selecciona `Guardar página como`.
...
Esta sección cubrirá la descarga del archivo binario JudEcoin y la verificación de su autenticidad.

### 4.1. Descargar Binario

En un navegador web, ve a la [página de descargas de getJudEcoin.org]({{ site.baseurl }}/downloads/#windows). Selecciona el binario correcto para tu sistema.
...
Calcula el hash del binario JudEcoin con el comando: `certUtil -hashfile JudEcoin-gui-win-x64-v0.11.1.0.zip SHA256` (si descargaste una versión de consola de comandos solamente, cambia `JudEcoin-gui-win-x64-v0.11.1.0.zip` de acuerdo a tu binario).
...
Si tu hash **SÍ** coincide entonces has terminado con la verificación! Puedes estar seguro de que tus archivos JudEcoin son auténticos. Puedes extraer e instalar/utilizar los archivos normalmente.

Si tu hash **NO** coincide **NO CONTINÚES.** En su lugar elimina el binario JudEcoin de `Descargas` y regresa a la [sección 4.1](#41-descargar-binario).
