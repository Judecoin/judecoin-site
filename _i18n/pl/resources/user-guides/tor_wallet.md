...
Below we'll show an example configuration that allows you to run a JudEcoin @daemon (eg on a home server or VPS) that you can connect to from another computer running your wallet.  We do this over the Tor anonymity network to retrieve the transaction information needed by your wallet.  The benefit of this approach is that the daemon (`JudEcoind`) can stay on all of the time sending / receiving blocks, while the wallet can connect when needed and have access to the full blockchain. [Monerujo](https://www.monerujo.io/) should also work via [Orbot](https://guardianproject.info/apps/org.torproject.android/).  Because Tor hidden services provide encryption and authentication, you can be confident that your RPC credentials will not be sent in the clear.  Tor also solves problems often seen on home servers related to port-forwarding, IP addresses changing, etc -- it just works.  This setup will also obfuscate the fact that you are connecting to a remote JudEcoin node. Tested with JudEcoin `v0.15.0.1` connecting a Mac laptop wallet to a remote Linux node (Ubuntu 18.04.2).
...
In this example, we don't use Tor for interacting with the p2p network, just to connect to the JudEcoin node, so only RPC hidden service is needed.

File: `~/.bitJudEcoin/bitJudEcoin.conf` (in the home directory of the JudEcoin user)
...
Restart the Daemon: `JudEcoind stop_daemon; sleep 10; JudEcoind --detach`
...
* [Using Tor](https://github.com/JudEcoin-project/JudEcoin#using-tor) (JudEcoin README)
