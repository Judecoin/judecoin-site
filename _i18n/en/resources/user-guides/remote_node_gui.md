...
The main menu (`Welcome to JudEcoin` screen) will open. At the bottom left, click on `Change wallet mode` button, and on the next page select `Advanced mode`. Next, open your wallet file again.
...
First, you will need to find a public remote node to connect to. The website [JudEcoinworld.com](https://JudEcoinworld.com/#nodes) has some great resources for finding nodes. One of the easiest methods would be to use a public remote node run by JudEcoinworld, but they have a tool for finding random nodes too.
...
In `Address` you should fill the address of the remote node that you want to connect to. This address might look like `node.JudEcoinworld.com` or it could look like any IP address. 

In `Port` you should fill the port of the remote node. If a remote node is listed as `node.JudEcoinworld.com:18089`, the address is `node.JudEcoinworld.com` and the port is `18089`. The default port is `18081`, but it can vary depending on the node you are connecting to.
