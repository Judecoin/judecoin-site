...
#### Resource for JudEcoin Binaries:  [JudEcoin Binaries]({{ site.baseurl_root }}/downloads/)
...
- Create a Linux or Windows bootable disk, and make sure you have the JudEcoin binaries on the same disk or on a second disk (for Linux make sure you have also downloaded copies of the dependencies you will need, libboost1.55 and miniupnpc for instance)
...
- Copy the JudEcoin binaries to a RAM disk (/dev/shm in Linux, Windows bootable ISOs normally have a Z: drive or something)

- Don't run the JudEcoin @daemon. Instead, using the command line, use JudEcoin-wallet-cli to create a new JudEcoin @account
...
Related:  [Offline Account Generator](http://JudEcoinaddress.org/)
